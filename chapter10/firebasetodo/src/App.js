...
function App() {
  ...
  //useEffect to invoke the fetchItems() immediately after the first render
  useEffect(() => {
    fetchItems();
  }, [])
  ...
  const fetchItems = () => {
    fetch('https://todolist-9cff5-default-rtdb.firebaseio.com/items/.json')
  ...
  const addKeys = (data) => {
    ...
    const valueKeys = Object.values(data).map((item, index) =>  //Object.values() return an array of the values objects
  ...
  const deleteTodo = (id) => {
    fetch(`https://todolist-9cff5-default-rtdb.firebaseio.com/items/${id}.json`,
   {
      method: 'DELETE',
    })
    .then(response => fetchItems())
